...
def register_user(user:UserRegister):
    ...
    if existing_user:
        if existing_user.get("otp_verified"):
            cursor.close()
            connection.close()
            raise HTTPException(400,"Email already registered")
        else:
            # Re-use existing unverified user entry (SAME ID)
...
def get_user_profile(email:str):
    connection=get_connection()
    cursor=connection.cursor(dictionary=True)
    cursor.execute("SELECT id, name, email, age FROM users WHERE email=%s", (email,))
    user = cursor.fetchone()
    cursor.close()
    connection.close()
    if not user:
...
async def encrypt_file(email:str=Form(...),file:UploadFile=File(...)):
    ...
    cursor.execute("""
        INSERT INTO files(user_id,file_name,file_format,file_path,file_type,decryption_key,file_size)
        VALUES(%s,%s,%s,%s,%s,%s,%s)
    """,(user["id"],filename,file_format,path,"encrypted",decryption_key,file_size))
    connection.commit()
...
def get_encrypted_files(email:str):
    ...
    cursor.execute("""
        SELECT id,file_name,file_path,file_size,DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s') as created_at
        FROM files
...
def get_user_history(email:str):
    ...
    if not user:
        ...
        return {"history": []}
    cursor.execute("""
        SELECT id, file_name, file_type, file_path, file_format, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s') as created_at
        FROM files
        WHERE user_id=%s
        ORDER BY created_at DESC
    """,(user["id"],))
...
def get_notifications(email:str):
    ...
    cursor.execute("""
        SELECT message,DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s') as created_at
        FROM notifications
        WHERE user_id=%s
        ORDER BY created_at DESC
    """,(user["id"],))
...
# NEW: My Files route if needed by Android
@app.get("/my-files/{email}")
def get_my_files(email:str):
    return get_encrypted_files(email)
